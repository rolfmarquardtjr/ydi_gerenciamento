...
interface MaintenanceTestConfigProps {
  config: any;
  onSave: (config: any) => void;
}

const MaintenanceTestConfig: React.FC<MaintenanceTestConfigProps> = ({ config: providedConfig, onSave }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center">
        ...
        <div>
          <h3 className="text-lg font-semibold">Teste de Manutenção</h3>
          <p className="text-sm text-gray-600">
            Configure os parâmetros do teste de conhecimento em manutenção
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input
            type="number"
            min="1"
            value={localConfig.questions}
            onChange={(e) => handleSettingChange('questions', Number(e.target.value))}
            className="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500"
          />
        </div>
      </div>
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        <p className="text-sm text-gray-600">
          O teste avalia o conhecimento básico sobre manutenção preventiva e 
...
export default MaintenanceTestConfig;