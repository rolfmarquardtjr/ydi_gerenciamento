...
import { AlertTriangle, Clock, MapPin, Activity, Brain } from 'lucide-react';
    ...
    <div className="bg-white rounded-lg shadow-sm p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          ...
          <h3 className="text-lg font-semibold">Análise de Risco IA</h3>
        </div>
        <div className={`${getScoreBg(score)} px-4 py-2 rounded-lg`}>
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className={getScoreColor(factors.eventFrequency)}>
                {factors.eventFrequency}%
              </span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className={`h-2 rounded-full ${getScoreBg(factors.eventFrequency)} transition-all`}
                style={{ width: `${factors.eventFrequency}%` }}
              />
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className={getScoreColor(factors.eventSeverity)}>
                {factors.eventSeverity}%
              </span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className={`h-2 rounded-full ${getScoreBg(factors.eventSeverity)} transition-all`}
                style={{ width: `${factors.eventSeverity}%` }}
              />
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className={getScoreColor(factors.timePatterns)}>
                {factors.timePatterns}%
              </span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className={`h-2 rounded-full ${getScoreBg(factors.timePatterns)} transition-all`}
                style={{ width: `${factors.timePatterns}%` }}
              />
            </div>
          </div>
          ...
          <div>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className={getScoreColor(factors.locationRisk)}>
                {factors.locationRisk}%
              </span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className={`h-2 rounded-full ${getScoreBg(factors.locationRisk)} transition-all`}
                style={{ width: `${factors.locationRisk}%` }}
              />
            </div>
          </div>
        </div>
